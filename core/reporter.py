"""Handles all report output formats: CSV bundles, a self-contained HTML page, and an optional PDF.

PDF output requires the reportlab package; all other formats use the standard library only.
The HTML report and PDF both include a chain-of-custody audit block as the first section.
"""
...
def export_html_report(
    ...
    # Escape before inserting into HTML to prevent injection in the report file
    ...
    # Chain-of-custody block goes at the top so it's the first thing a reviewer sees
    ...
    # Cap rows in the HTML to keep the file size manageable; CSV has the full set
    cap = 500
    if len(metas) > cap:
    ...
    # Hash table is also capped to avoid enormous HTML files on large scans
    cap = 200
    if len(hashes) > cap:
    ...
    # Timeline is capped for the same reason as metadata — export CSV for the full set
    cap = 1000
    if len(events) > cap:
...
def export_pdf_report(
    ...
    # Chain-of-custody audit block mirrors the HTML version so both formats carry the same evidence
    if scan_start and scan_end:
        duration = str(scan_end - scan_start).split('.')[0]
        audit_data = [
            ['Field', 'Value'],
            ...
            ['Files scanned', str(len(metas))],
            ['Scan started', scan_start.strftime('%Y-%m-%d %H:%M:%S')],
            ['Scan ended', scan_end.strftime('%Y-%m-%d %H:%M:%S')],
            ['Duration', duration],
        ]
        story.append(Paragraph("Audit Log — Chain of Custody", styles['Heading2']))
        t = Table(audit_data, colWidths=[150, 350])
    ...
    # Timeline is capped at 500 rows to keep PDF file size reasonable
    if events:
