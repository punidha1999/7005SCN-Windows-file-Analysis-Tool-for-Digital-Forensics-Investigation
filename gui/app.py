...
class SquareToggle(tk.Button):
    """[✓] / [ ] bracket-style toggle synced to a BooleanVar."""
    def __init__(self, parent, text, variable, command=None, **kw):
        self._var      = variable
        self._label    = text
        self._callback = command
        super().__init__(parent, command=self._toggle,
    ...
    def _refresh(self):
        on = self._var.get()
        self.config(
            text=f'[✓] {self._label}' if on else f'[ ] {self._label}',
            fg=ACCENT        if on else '#4a5568',
            bg=BG2           if on else BG,
...
class ForensiScanApp:
    ...
    def _build_ui(self):
        ...
        # Non-Windows uses inode-change time for st_ctime, not actual file creation
        ...
        SquareToggle(ctrl, text="Recursive",  variable=self._recursive).pack(side='left')
        SquareToggle(ctrl, text="Hashing",    variable=self._run_hashes).pack(side='left', padx=4)
        SquareToggle(ctrl, text="Signatures", variable=self._run_sigs).pack(side='left')
        SquareToggle(ctrl, text="Hidden",     variable=self._run_hidden).pack(side='left', padx=4)
        SquareToggle(ctrl, text="Entropy",    variable=self._run_entropy).pack(side='left')
        ttk.Button(ctrl, text="▶  Scan",  command=self._start_scan).pack(side='left', padx=8)
        # Allow the investigator to wipe results before starting a new case
    ...
    def _make_metadata_tab(self, nb):
        ...
        # Risk column at the end lets investigators sort immediately by threat level
    ...
    def _make_hashes_tab(self, nb):
        ...
        # Hash set loader sits above the table so it's visible before results come in
    ...
    def _make_hidden_tab(self, nb):
        ...
        # NTFS ADS enumeration calls the Windows kernel directly and won't work on other platforms
        if platform.system() != 'Windows':
            tk.Label(
                frame,
    ...
    def _make_timeline_tab(self, nb):
        ...
        SquareToggle(filt_row, text="Include accessed events",
                     variable=self._tl_incl_accessed,
                     command=self._rebuild_timeline).pack(side='left', padx=12)

        # Date range filter — lets investigators narrow to a specific window of activity
        range_row = ttk.Frame(frame)
        range_row.grid(row=1, column=0, sticky='ew', padx=4, pady=2)
        ttk.Label(range_row, text="From:").pack(side='left')
        self._tl_from = tk.StringVar()
        ttk.Entry(range_row, textvariable=self._tl_from, width=20).pack(side='left', padx=4)
        ttk.Label(range_row, text="To:").pack(side='left')
        self._tl_to = tk.StringVar()
        ttk.Entry(range_row, textvariable=self._tl_to, width=20).pack(side='left', padx=4)
        ttk.Button(range_row, text="Apply Range",
                   command=self._apply_date_range).pack(side='left', padx=4)
        self._tl_range_label = ttk.Label(range_row, text="")
        self._tl_range_label.pack(side='left', padx=8)

        self._tl_day_summary = ttk.Label(frame, text="", anchor='w')
        self._tl_day_summary.grid(row=2, column=0, sticky='ew', padx=8, pady=(0, 2))

        cols = [('Timestamp', 155), ('Event', 75), ('Path', 400),
                ('Size', 80), ('Ext', 55), ('Hidden', 55)]
        self._tl_table = TableView(frame, cols)
        self._tl_table.grid(row=3, column=0, sticky='nsew')

        # Separate table below the main timeline shows files whose timestamps look tampered
        tk.Label(
            frame,
            text='Timestamp Anomalies (Modified before Created — possible tampering)',
            fg=WARN, bg=BG, font=FONT_B, anchor='w'
        ).grid(row=4, column=0, sticky='ew', padx=8, pady=(8, 2))
        anomaly_cols = [('Path', 350), ('Created', 155), ('Modified', 155),
                        ('Delta', 100), ('Note', 280)]
        self._anomaly_table = TableView(frame, anomaly_cols)
        self._anomaly_table.grid(row=5, column=0, sticky='nsew')
        return frame

    def _make_entropy_tab(self, nb):
        # Entropy tab shows per-file randomness scores to help spot encrypted or packed files
        frame = ttk.Frame(nb)
        frame.rowconfigure(0, weight=1)
        frame.columnconfigure(0, weight=1)
    ...
    def _run_scan(self, target: str):
        # Record when the scan begins so duration appears in the audit log
        self._scan_start = datetime.datetime.now()
        try:
            # Metadata first — everything else depends on the file list
            self._metas = []
            ...
            # Check magic bytes against declared extensions
            self._sigs = []
            ...
            # Cryptographic hashes for integrity and deduplication
            self._hashes = []
            self._duplicates = {}
            ...
            # Look for ADS streams and suspicious attribute combinations
            self._hidden = []
            ...
            # Entropy analysis — high scores suggest encrypted or packed content
            self._entropy = []
            ...
            # Re-draw metadata table now that risk scores can be calculated
            self._populate_metadata()

            # Build the timeline and check for impossible timestamp sequences
            self._rebuild_timeline()

            # If the investigator loaded a hash set before scanning, run the match now
            if self._hashset:
            ...
            self._set_status("Permission denied — try running as administrator.")
            messagebox.showerror("Permission Error",
                                 "Access denied. Try running ForensiScan as administrator.")
        except FileNotFoundError:
            self._set_status("Directory not found — it may have been moved or deleted.")
            ...
            # Record when the scan finishes to complete the duration calculation
    ...
    def _populate_metadata(self):
        ...
        def _do():
            ...
            for m in self._metas:
                # Combine signals from signatures and entropy into a single risk score per file
    ...
    def _populate_hashes(self):
        ...
        def _do():
            ...
            for h in self._hashes:
                tag = 'orange' if h.path in dup_paths else ''
                self._hash_table.insert(
                    [h.path, h.md5, h.sha1, h.sha256, h.size, ''], tag)
        self.root.after(0, _do)

    def _populate_hidden(self):
        def _do():
            self._hidden_table.clear()
            for r in hid_mod.filter_suspicious(self._hidden):
    ...
    def _populate_entropy(self):
        # Paint high-entropy rows red so they stand out immediately
        def _do():
    ...
    def _rebuild_timeline(self):
        ...
        # Detect modified-before-created conditions that indicate timestamp manipulation
    ...
    def _populate_anomalies(self):
        # Every anomaly row is red because all represent potential evidence of tampering
        def _do():
    ...
    def _update_day_summary(self):
        if not self._events:
            return
        groups = tl_mod.group_by_day(self._events)
        n_days = len(groups)
        if n_days:
            dates = sorted(groups.keys())
            summary = f"Activity spans {n_days} day(s): {dates[0]} to {dates[-1]}"
        else:
            summary = ""
        self.root.after(0, lambda: self._tl_day_summary.config(text=summary))

    def _apply_date_range(self):
        from_str = self._tl_from.get().strip()
        to_str = self._tl_to.get().strip()
        try:
            start = datetime.datetime.strptime(from_str, '%Y-%m-%d')
            end = datetime.datetime.strptime(to_str, '%Y-%m-%d').replace(
                hour=23, minute=59, second=59)
        except ValueError:
            messagebox.showerror("Date Error",
                                 "Enter dates as YYYY-MM-DD (e.g. 2024-01-15)")
            return
        kind = self._tl_kind.get()
        events = self._events if kind == 'all' else tl_mod.filter_by_kind(self._events, kind)
        filtered = tl_mod.filter_by_range(events, start, end)
        self._tl_range_label.config(text=f"{len(filtered)} events in range")
    ...
    def _reset(self):
        ...
        self._tl_day_summary.config(text='')
        self._tl_range_label.config(text='')
